...
public class VentaPDF {
    ...
    public void generarFacturaPDF() {
        try {
            ...
            PdfPTable tablaProducto = new PdfPTable(6); // Ahora 6 columnas (incluyendo IVA y Precio Final)
            tablaProducto.setWidthPercentage(100);
            tablaProducto.getDefaultCell().setBorder(0);
            //tamaño de celdas con mayor separación entre columnas
            float[] ColumnaProducto = new float[]{12f, 35f, 15f, 12f, 13f, 13f}; // Aumentamos el tamaño de las columnas
            tablaProducto.setWidths(ColumnaProducto);
            tablaProducto.setHorizontalAlignment(Element.ALIGN_LEFT);
            PdfPCell producto1 = new PdfPCell(new Phrase("Cantidad: ", negrita));
            PdfPCell producto2 = new PdfPCell(new Phrase("Descripcion: ", negrita));
            PdfPCell producto3 = new PdfPCell(new Phrase("Precio Unitario: ", negrita));
            PdfPCell producto4 = new PdfPCell(new Phrase("Subtotal: ", negrita));
            PdfPCell producto5 = new PdfPCell(new Phrase("IVA: ", negrita));
            PdfPCell producto6 = new PdfPCell(new Phrase("Precio Final: ", negrita));
            ...
            //agregar color al encabezado de la tabla
            ...
            double totalPagar = 0; // Variable para el total a pagar

            //Recorremos la tabla de productos y agregamos las filas
            for (int i = 0; i < InterFacturacion.jTable_productos.getRowCount(); i++) {
                ...
                String precioUnitario = InterFacturacion.jTable_productos.getValueAt(i, 3).toString();
                String iva = InterFacturacion.jTable_productos.getValueAt(i, 6).toString(); // IVA ya calculado
                String precioFinal = InterFacturacion.jTable_productos.getValueAt(i, 7).toString(); // Precio final ya calculado

                // Sumamos el precio final al total
                totalPagar += Double.parseDouble(precioFinal);

                // Agregamos los valores a la tabla
                tablaProducto.addCell(cantidad);
                tablaProducto.addCell(producto);
                tablaProducto.addCell(precioUnitario);
                tablaProducto.addCell(String.format("%.2f", Double.parseDouble(precioUnitario) * Integer.parseInt(cantidad)));
                tablaProducto.addCell(iva);
                tablaProducto.addCell(precioFinal);
            }
